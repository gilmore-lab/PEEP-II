function status = handle_keypress(session, environment, status)
...
[pressed, secs, firstPress, ~] = KbCheck(environment.internal_kbd_index);
if pressed
    ...
    if firstPress(environment.leftArrowKey) % Move rating lower
        fprintf('Highlighted %i\n', status.highlighted_index);
    ...
    if firstPress(environment.enterKey) % Enter and save rating
        ...
        if (status.snd_index + 1 <= session.n_snds)
            ...
            [this_snd, ~, ~] = load_peep_sound(session.this_run_data.File(session.snd_index));
